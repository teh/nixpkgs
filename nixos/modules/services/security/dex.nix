...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    users.extraUsers = singleton {
      name = "dex";
      uid = config.ids.uids.dex;
      description = "dex user";
    };

    users.extraGroups = singleton {
      name = "dex";
      gid = config.ids.gids.dex;
    };
    ...
    systemd.services.dex-overlord = {
      ...
      serviceConfig = {
        ExecStart = "${pkg.bin}/bin/dex-overlord";
        Restart = "on-failure";
        RestartSec = "10s";
    ...
    systemd.services.dex-worker = {
      ...
      environment = {
        ...
        DEX_WORKER_HTML_ASSETS = "${pkg}/share/go/src/github.com/coreos/dex/static/html/";
        DEX_WORKER_CLIENTS = "${clientConfigFile}";
        DEX_WORKER_CONNECTORS = "${connectorConfigFile}";
      };

      serviceConfig = {
        ExecStart = "${pkg.bin}/bin/dex-worker";
        ExecStartPost = "${pkg.bin}/bin/dexctl --db-url ${cfg.overlordDBURL} set-connector-configs ${connectorConfigFile}";
        Restart = "on-failure";
        RestartSec = "10s";
        StartLimitInterval = "1min";
      };
